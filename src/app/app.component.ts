...
export class AppComponent {
  ...
  loadChat(){
    setInterval(
      () => {
        const messageImported = this.http.get('https://easychatmk1.firebaseio.com/messages.json').subscribe((data: MessageObject) => this.messageImported2 = data);
        this.messages = [];
        for (let key in this.messageImported2) {
          this.messages.push(this.messageImported2[key]);
        } }, 2000);
  }
